...
func TestGetPriceBinanceSuccess(t *testing.T) {
	...
	if prices.Values["USDT"] != 525.742 {
		t.Fatalf("Expected USDT=525.742, got %v", prices.Values["USDT"])
	}
	// BUY (sell orders avg): (515 + 525.5 + 513.8 + 545.12 + 543) * 5 / 25 = 528.484
	if prices.Values["USDT_BINANCE_BUY"] != 528.484 {
		t.Fatalf("Expected USDT_BINANCE_BUY=528.484, got %v", prices.Values["USDT_BINANCE_BUY"])
	}
	// SELL (buy orders avg): (505 + 514 + 523 + 532 + 541) * 5 / 25 = 523.0
	if prices.Values["USDT_BINANCE_SELL"] != 523.0 {
		t.Fatalf("Expected USDT_BINANCE_SELL=523.0, got %v", prices.Values["USDT_BINANCE_SELL"])
	}
...
func TestGetPriceBinanceNegativePricesFiltered(t *testing.T) {
	...
	if _, ok := prices.Values["USDT_BINANCE_SELL"]; !ok {
		t.Fatal("expected USDT_BINANCE_SELL key to be present")
	}
	if _, ok := prices.Values["USDT_BINANCE_BUY"]; !ok {
